			...
			<span class="class_auto">'. $cards[$i]['class'] .'</span>
				...
				<span class="card__title">Характеристики</span>
				 ...
				 <span name="get-color">Цвет: '. $cards[$i]['color'] .'</span>
					...
					';
					if (isset($_SESSION['alogin'])) echo '<button type="submit" id="open-modal-btn" class="card__add" >Удалить</button>';
					else echo '<a class="card__add" href="rent.php?brend='. $cards[$i]['brend'] .'&model='. $cards[$i]['model'] .'&id_auto='. $cards[$i]['id_auto'] .'&cena='. $cards[$i]['cena'] .'"><input type="hidden" id="open-modal-btn">Арендовать</input>';
					echo '
					</a>				
					</form>
					</div>
